function [ segm, prior ] = graphcut_segm(I, area, ...
    K, alpha, sigma,CovIn,MaskMeth,threshold, noise)
% area = [ 52, 77, 283, 142 ]

[h,w,c] = size(I);

if strcmpi('manual',MaskMeth)
    dw = area(3) - area(1) + 1;
    dh = area(4) - area(2) + 1;
    mask = uint8([zeros(area(2)-1,w); zeros(dh,area(1)-1), ones(dh,dw), ...
        zeros(dh,w-area(3)); zeros(h-area(4),w)]);
elseif strcmpi('poly',MaskMeth)
    maximizeSubFigs()
    mask = roipoly(I);
    close;
end

% add noise to avoid segmentation instabilities
if strcmpi('on',noise)
    I = imnoise(I,'gaussian',1e-10);
end

Ivec = single(reshape(I, size(I,1)*size(I,2), 3));

grey = single(rgb2gray(I));
h = fspecial('gaussian', [7,7], 0.5);
grey = imfilter(grey, h);
h = fspecial('sobel');
dx = imfilter(grey, h/4);
dy = imfilter(grey, h/4');
grad = sqrt(dx.^2 + dy.^2);
edge = (alpha*sigma)*ones(size(grey)) ./ (grad + sigma);

tic
for l=1:3
    fprintf('Find Gaussian mixture models...\n');
    fprob = mixture_prob(I, K, 10, mask,CovIn);
    bprob = mixture_prob(I, K, 10, 1-mask,CovIn);
    prior = reshape(fprob ./ (fprob + bprob), size(I,1), size(I,2), 1);
    toc
    
    fprintf('Find minimum cut...\n');
    [u, erriter, i] = cmf_cut(prior, edge);
    mask = uint8(u>threshold);
    toc
    
end

segm = int16(u>0.5) + 1;

